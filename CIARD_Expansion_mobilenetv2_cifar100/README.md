...
AutoAttack单独记录：**24.95%**。补充材料AA表是CIFAR-10，不套用为CIFAR-100基线。均值不含AA，先计算再舍入，不代表联合最坏情况准确率。
...
本目录包含24份Python、原requirements及两份Slurm模板；Python、CFG、prefix和评测实现与已测run逐字一致。脚本只适配工作/日志绝对路径与job-name，默认compute-4、rtx4090、训练2卡/评测1卡、4CPU/16GB。两组已完成评测实际使用compute-2；模板默认节点不等于历史实际执行节点。compute-2仍需注意实际7卡与GRES登记8卡的差异，保留原CUDA设备检查。
