...
def teacher_reliability_score(teacher_logits, labels, temperature=1.0,
                               tau_m=2.0, floor=0.1):
    """Return a detached per-sample teacher reliability weight in [floor, 1]."""
    with torch.no_grad():
        ...
        return floor + correct * (full_weight - floor)


def sample_epsilon_curriculum(epoch, total_epochs, eps_max=8.0/255.0,
                               eps_min=1.0/255.0):
    """Sample the SAA epsilon from a weak-to-strong Beta curriculum."""
    progress = float(epoch) / float(max(total_epochs, 1))
    if progress < 0.33:
        alpha, beta = 2.0, 5.0
    elif progress < 0.67:
        transition = (progress - 0.33) / 0.34
        alpha = 2.0
        beta = 5.0 + transition * (2.0 - 5.0)
    else:
        transition = (progress - 0.67) / 0.33
        alpha = 2.0 + transition * (5.0 - 2.0)
        beta = 2.0
